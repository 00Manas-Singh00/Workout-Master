...
const Profile = ({ isDarkMode }) => {
  ...
  useEffect(() => {
    ...
    const weeklyPlan = localStorage.getItem('aiWeeklyPlan');
    setShowWeeklyPlan(!!weeklyPlan);
  }, []);
  ...
  const handleGenerateAIWorkout = async (planType) => {
    try {
      const userProfile = {
      ...
      const response = await getAIRecommendation(planType, userProfile);
      console.log('AI Recommendation received:', response);
      
      if (!response.success) {
        toastError(response.message || 'Failed to generate AI recommendation');
        return;
      }
      ...
      if (recommendation.type === 'session') {
        // Store AI-generated workout in localStorage or context
        localStorage.setItem('aiGeneratedWorkout', JSON.stringify(recommendation.session));
        toastSuccess('AI workout generated! Redirecting to workout...');
        // Add small delay to ensure localStorage is set before navigation
        setTimeout(() => {
          navigate('/workout');
        }, 100);
      } else if (recommendation.type === 'weekly') {
        // Store the weekly plan
        localStorage.setItem('aiWeeklyPlan', JSON.stringify(recommendation.plan));
        setShowWeeklyPlan(true);
        toastSuccess('Weekly plan generated!');
      } else {
        toastError('Invalid response from AI service');
      }
    } catch (error) {
      console.error('AI Workout generation error:', error);
      toastError(error.message || 'Failed to generate AI recommendation');
    }
  };
