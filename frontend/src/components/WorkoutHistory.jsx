...
const WorkoutHistory = ({ isDarkMode }) => {
  ...
  const getStatusLabel = (workout) => {
    if (workout.status === 'skipped') return 'Skipped';
    if (workout.completed || workout.status === 'completed') return 'Completed';
    return 'In Progress';
  };

  const getStatusClasses = (workout) => {
    const status = getStatusLabel(workout);
    if (status === 'Completed') {
      return isDarkMode
  ...
  const handleClearHistory = async () => {
    if (history.length === 0) {
      toastError('No workouts to clear');
      return;
    }
    
    if (window.confirm(`Are you sure you want to delete all ${history.length} workouts? This action cannot be undone.`)) {
      try {
        // Delete all workouts one by one
        const deletePromises = history.map(workout => removeWorkout(workout._id));
        await Promise.all(deletePromises);
        toastSuccess(`Successfully cleared ${history.length} workouts`);
        setActiveWorkout(null);
      } catch (error) {
        toastError('Failed to clear workout history');
                  ...
                  <div className="flex justify-between items-start">
                    <div>
                      ...
                      <div className="flex gap-2 items-center mt-1">
                        <span className={`px-2 py-0.5 text-xs rounded-full ${getStatusClasses(workout)}`}>
                    ...
                    <div key={workout._id}>
                      <div className="mb-6">
                        ...
                        <div className="flex items-center gap-3">
                          <span className={`px-2.5 py-1 text-sm rounded-full ${getStatusClasses(workout)}`}>
                            {getStatusLabel(workout)}
                          </span>
