...
const mapMuscleNames = (muscleId) => {
    ...
    if (muscleId === 'lats') return ['back', 'lats'];
    if (muscleId === 'delts') return ['shoulders', 'delts'];
    if (muscleId === 'core') return ['core', 'abs'];
...
export const generateWorkout = ({ muscleGroups, workoutType, goal }) => {
    ...
    if (matchingExercises.length === 0) {
        ...
        matchingExercises = Object.entries(FALLBACK_EXERCISES).map(([name, data]) => ({
            name: name,
            ...data
        }));
